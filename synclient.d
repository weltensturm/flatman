...
struct Parameter {
    ...
    ParaType type;         /* Type of parameter */
...
static double parse_cmd(char* cmd, Parameter** par){
    char *eqp = strchr(cmd, '=');
    *par = NULL;
    if (eqp) {
        int j;
...
static XDevice* dp_get_device(Display* dpy){
    XDevice *dev = NULL;
...
static void dp_set_variables(Display* dpy, XDevice * dev, int argc, char*[] argv, int first_cmd){
    int i;
    double val;
    Parameter *par;
...
static void dp_show_settings(Display* dpy, XDevice* dev){
    int j;

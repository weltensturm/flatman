...
alias IconClicked = Event!("IconClicked", void function());
...
void main(string[] args){
	...
	auto addIcon = {
		auto tray = X.GetSelectionOwner(wm.displayHandle, Atoms._NET_SYSTEM_TRAY_S0);
		if(tray){
			icon = new TrayIcon(pa, tray);
			wm.add(icon);
			main.icon = icon;
			icon.dock;
		}
	...
	X.SelectInput(wm.displayHandle, DefaultRootWindow(wm.displayHandle), StructureNotifyMask);
	WindowClientMessage ~= (w, e){
		if(e.message_type == Atoms.MANAGER && e.data.l[1] == Atoms._NET_SYSTEM_TRAY_S0){
			if(icon)
				icon.close;
			addIcon();
		}
	...
	while(wm.hasActiveWindows){
		if(icon && !icon.hidden && icon.redraw)
			icon.onDraw;
		if(!main.hidden){
			main.onDraw;
		}
...
class TrayIcon: ws.wm.Window {
	...
	this(Pulseaudio pa, WindowHandle tray){
		this.pa = pa;
		this.tray = tray;
		...
		X.SelectInput(wm.displayHandle, window, eventMask);
	...
	void onClientMessage(XClientMessageEvent* e){
		auto name = X.GetAtomName(wm.displayHandle, e.message_type);
		writeln(name, ' ', *e);
		X.Free(name);
	...
	@WindowReparent
	void onReparent(WindowHandle parent){
		if(parent != tray){
			writeln("closing");
			close;
		}
	...
	void dock(){
		...
		tray = X.GetSelectionOwner(wm.displayHandle, Atoms._NET_SYSTEM_TRAY_S0);
...
class AudioPanel: ws.wm.Window {
	...
	override void onShow(){
		...
		super.onShow;
		X.Sync(wm.displayHandle, False);
		X.RaiseWindow(wm.displayHandle, windowHandle);
	}
...
class SinkRow: Base {
	...
	this(Pulseaudio pa, Device sink){
		...
		slider = addNew!CustomSlider(sink.type == sink.Type.sink ? yellow : green, this);
...
class CustomSlider: Slider {
	...
	this(float[3] color, SinkRow container){
		this.color = color;
	...
	override void onDraw(){
		...
		float[3] color = color[]*(container.inUse ? 1 : 0.3);
		draw.setColor(color);
